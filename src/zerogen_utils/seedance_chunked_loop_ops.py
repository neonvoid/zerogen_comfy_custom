"""Pure helpers for SeedanceChunkedLoop.
...
by inline-copy for an internal ticket's runtime-test window — once both nodes are
...
Shared helpers (inline-copy of kling_chunked_loop_ops):
  ...
  - compute_original_from_slowed — reverse the MatchInterpFrames math
...
# SeedancePrep for ref-video duration, BUT for the chunked loop the
...
def validate_filename_prefix(prefix: str) -> str:
    """Reject paths/spaces/dots. Matches ShotSaverPath custom-name charset."""
    s = (prefix or "").strip()
    if not s:
        raise ValueError("[SeedanceChunkedLoop] filename_prefix is empty.")
    if not VALID_PREFIX_RE.match(s):
        raise ValueError(
            f"[SeedanceChunkedLoop] filename_prefix {s!r} contains disallowed "
...
def validate_output_dir(output_dir: str) -> Path:
    ...
    if not s:
        raise ValueError(
            "[SeedanceChunkedLoop] output_dir is empty — wire ShotSaverPath "
            "or provide an absolute path."
        )
    p = Path(s).expanduser()
    if not p.is_absolute():
        raise ValueError(
            f"[SeedanceChunkedLoop] output_dir must be absolute, got {s!r}."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"[SeedanceChunkedLoop] output_dir {p} is not writable: "
...
def preflight_collision_check(
    ...
    if policy == "fail":
        raise FileExistsError(
            f"[SeedanceChunkedLoop] {len(head_collisions)} chunk file(s) already "
    ...
    for n in range(2, 101):
        ...
        if not _collisions_for(candidate):
            print(
                f"[SeedanceChunkedLoop] filename collision auto-rename: "
                f"{prefix!r} → {candidate!r} (existing files preserved)"
            )
            return candidate
    raise RuntimeError(
        f"[SeedanceChunkedLoop] auto_rename exhausted 100 attempts on prefix "
...
def validate_chunk_seconds(target_chunk_seconds: int) -> int:
    ...
    if not isinstance(target_chunk_seconds, int):
        raise ValueError(
            f"[SeedanceChunkedLoop] target_chunk_seconds must be int, got "
    ...
    if target_chunk_seconds < SEEDANCE_MIN_CHUNK_SECONDS or target_chunk_seconds > SEEDANCE_MAX_CHUNK_SECONDS:
        raise ValueError(
            f"[SeedanceChunkedLoop] target_chunk_seconds={target_chunk_seconds} "
    ...
    if len(ranges) >= 2:
        ...
        if last_seconds < SEEDANCE_MIN_CHUNK_SECONDS:
            ...
            if combined_seconds <= SEEDANCE_MAX_CHUNK_SECONDS:
                ...
                raise ValueError(
                    f"[SeedanceChunkedLoop] last chunk would be "
...
def compute_original_from_slowed(slowed_count: int, slowdown_factor: int) -> int:
    """Reverse MatchInterpFrames math: slowed = (original - 1) * factor + 1.
    ...
    if slowdown_factor < 1:
        raise ValueError(
            f"[SeedanceChunkedLoop] slowdown_factor must be >= 1, got {slowdown_factor}"
        )
    if slowed_count < 1:
        raise ValueError(
            f"[SeedanceChunkedLoop] slowed_count must be >= 1, got {slowed_count}"
        )
    if slowdown_factor == 1:
        return slowed_count
    if (slowed_count - 1) % slowdown_factor != 0:
        raise ValueError(
            f"[SeedanceChunkedLoop] input frame count {slowed_count} is not "
            f"consistent with slowdown_factor={slowdown_factor}. Expected "
            f"(slowed - 1) % factor == 0. Did you run MatchInterpFrames "
...
def restore_proportional(images: torch.Tensor, original_count: int) -> torch.Tensor:
    ...
    Mirrors RetimeRestore._select_frames. Used for the global retime
...
def validate_seedance_upload_config_for_chunked(cfg: dict) -> dict:
    ...
    if not isinstance(cfg, dict):
        raise ValueError(
            f"[SeedanceChunkedLoop] upload_config must be a dict from "
            f"SeedancePrep, got {type(cfg).__name__}"
        )
    required_keys = ("uploaded_image_urls", "uploaded_video_url", "n_images")
    missing = [k for k in required_keys if k not in cfg]
    if missing:
        raise ValueError(
            f"[SeedanceChunkedLoop] upload_config missing keys: {missing}. "
            f"Wire from SeedancePrep's config output."
        )
    image_urls = cfg["uploaded_image_urls"]
    if not isinstance(image_urls, list):
        raise ValueError(
            f"[SeedanceChunkedLoop] upload_config['uploaded_image_urls'] "
            f"must be a list, got {type(image_urls).__name__}"
        )
    if cfg.get("uploaded_video_url") is not None:
        raise ValueError(
            "[SeedanceChunkedLoop] upload_config has a ref video uploaded "
            ...
            "reference_video_frames from SeedancePrep — keep only "
...
def validate_seedance_v2_upload_config_for_chunked(cfg: dict) -> dict:
    ...
    The native chunked loop (Zerogen_SeedanceNativeChunkedLoop_V2) operates
    ...
    if not isinstance(cfg, dict):
        raise ValueError(
            f"[SeedanceNativeChunkedLoop] upload_config must be a dict "
            f"from Zerogen_SeedancePrep_V2, got {type(cfg).__name__}"
        )
    required_keys = ("mode", "content")
    missing = [k for k in required_keys if k not in cfg]
    if missing:
        raise ValueError(
            f"[SeedanceNativeChunkedLoop] upload_config missing keys: "
            f"{missing}. Wire from Zerogen_SeedancePrep_V2's upload_config output."
        )
    mode = cfg.get("mode")
    if mode != "multimodal":
        raise ValueError(
            f"[SeedanceNativeChunkedLoop] upload_config has mode={mode!r}, "
            ...
            f"Zerogen_SeedancePrep_V2's `reference_images` slot (do NOT wire "
    ...
    if not isinstance(content, list):
        raise ValueError(
            f"[SeedanceNativeChunkedLoop] upload_config['content'] must "
    ...
    if pre_uploaded_videos:
        raise ValueError(
            f"[SeedanceNativeChunkedLoop] upload_config has "
            ...
            f"reference_video_frames from Zerogen_SeedancePrep_V2 — keep only "
    ...
    if not image_refs:
        raise ValueError(
            "[SeedanceNativeChunkedLoop] upload_config is Mode C but has "
            "no reference images in its content array. Mode C requires 1-9 "
            "image refs. Wire at least one frame to Zerogen_SeedancePrep_V2's "
    ...
    if len(image_refs) > 9:
        raise ValueError(
            ...
            f"reference_images batch size wired to Zerogen_SeedancePrep_V2."
        )
    ...
    for i, item in enumerate(image_refs):
        ...
        if not isinstance(url, str) or not url.strip():
            raise ValueError(
                f"[SeedanceNativeChunkedLoop] upload_config image ref "
                f"#{i} has missing or empty 'url' (got {url!r}). Wire valid "
                f"ref images through Zerogen_SeedancePrep_V2 — do not hand-build "
                f"the config."
            )
        role = item.get("role")
        if role != "reference_image":
            raise ValueError(
                ...
                f"only via Zerogen_SeedancePrep_V2.reference_images slot."
            )
    return cfg
